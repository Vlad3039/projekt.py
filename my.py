...
# Кулонівський закон: F = k * Q1 * Q2 / r^2

# стала Кулона (Н·м²/Кл²)
""" k = 8.99 * 10**9  

# введення даних
Q1 = float(input("Введи заряд Q1 (у кулонах): "))
Q2 = float(input("Введи заряд Q2 (у кулонах): "))
r = float(input("Введи відстань r (у метрах): "))

# обчислення сили
F = k * Q1 * Q2 / r**2

# результат
print(f"Сила взаємодії між зарядами: {F:.2e} Н")

# перевірка напряму сили
if Q1 * Q2 > 0:
    print("→ Заряди однойменні → відштовхуються.")
elif Q1 * Q2 < 0:
    print("→ Заряди різнойменні → притягуються.")
else:
    print("→ Один із зарядів = 0 → сили немає.")
